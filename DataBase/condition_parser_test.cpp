...
void TestParseCondition() {
    ...
    {
        istringstream iStream(R"(event != "sport event" AND event != "Wednesday")");
        shared_ptr<Node> root = ParseCondition(iStream);
    ...
    {
        istringstream iStream(R"(event == "holiday AND date == 2017-11-18")");
        shared_ptr<Node> root = ParseCondition(iStream);
    ...
    {
        istringstream iStream(R"(((event == "holiday" AND date == 2017-01-01)))");
        shared_ptr<Node> root = ParseCondition(iStream);
        Assert(root->Evaluate({ 2017, 1, 1 }, "holiday"), "Parse condition 16");
        Assert(!root->Evaluate({ 2017, 1, 2 }, "holiday"), "Parse condition 17");
    }
    {
        istringstream iStream(R"(date > 2017-01-01 AND (event == "holiday" OR date < 2017-07-01))");
        shared_ptr<Node> root = ParseCondition(iStream);
    ...
    {
        istringstream iStream(R"(((date == 2017-01-01 AND event == "holiday")))");
        shared_ptr<Node> root = ParseCondition(iStream);
        Assert(root->Evaluate({ 2017, 1, 1 }, "holiday"), "Parse condition 26");
        Assert(!root->Evaluate({ 2017, 1, 2 }, "holiday"), "Parse condition 27");
    }
    {
        istringstream iStream(R"(((event == "2017-01-01" OR date > 2016-01-01)))");
        shared_ptr<Node> root = ParseCondition(iStream);
